...
- A **SysAdmin account is auto-seeded** on startup (`Data/DbInitializer.cs`): email `supper@mitanz360.edu`, password `P@ssw0rd`. Use it to log in at `/Account/Login`. (Password reset "change after first login" is only a comment; it is not enforced.)
- This is Blazor Server: the initial HTML is a shell and real content renders over the SignalR/WebSocket circuit, so plain `curl` of a page will not show interactive content — use a browser to verify UI.
